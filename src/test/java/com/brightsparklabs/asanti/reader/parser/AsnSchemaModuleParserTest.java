...
import com.brightsparklabs.asanti.mocks.MockAsnSchemaTypeDefinition;
...
public class AsnSchemaModuleParserTest
{
    ...
    public void testParse_Exceptions() throws Exception
    {
        ...
        try
        {
            ...
            when(AsnSchemaTypeDefinitionParser
                    .parse("People", "SET OF Person"))
                    .thenReturn(mockedPeopleTypeDefinition);
    ...
    public void testParse_DocumentPdu() throws Exception
    {
        ...
        when(AsnSchemaImportsParser
                .parse(expectedImportsStatement))
        ...
        when(AsnSchemaTypeDefinitionParser
                .parse("Header", "SEQUENCE { published [0] PublishedMetadata }"))
                .thenReturn(mockedAsnSchemaTypeDefinitions.get(1));

        when(AsnSchemaTypeDefinitionParser
                .parse("Body", "SEQUENCE { lastModified [0] ModificationMetadata, prefix [1] Section-Note OPTIONAL, content [2] Section-Main, suffix [3] Section-Note OPTIONAL }"))
                .thenReturn(mockedAsnSchemaTypeDefinitions.get(2));

        when(AsnSchemaTypeDefinitionParser
                .parse("Footer", "SEQUENCE { author [0] Person }"))
                .thenReturn(mockedAsnSchemaTypeDefinitions.get(3));

        when(AsnSchemaTypeDefinitionParser
                .parse("PublishedMetadata", "SEQUENCE { date [1] GeneralizedTime, country [2] OCTET STRING OPTIONAL }"))
                .thenReturn(mockedAsnSchemaTypeDefinitions.get(4));

        when(AsnSchemaTypeDefinitionParser
                .parse("ModificationMetadata", "SEQUENCE { date [0] Date, modifiedBy [1] Person }"))
                .thenReturn(mockedAsnSchemaTypeDefinitions.get(5));

        when(AsnSchemaTypeDefinitionParser
                .parse("Section-Note", "SEQUENCE { text [1] OCTET STRING }"))
                .thenReturn(mockedAsnSchemaTypeDefinitions.get(6));

        when(AsnSchemaTypeDefinitionParser
                .parse("Section-Main", "SEQUENCE { text [1] OCTET STRING OPTIONAL, paragraphs [2] SEQUENCE OF Paragraph }"))
    ...
    public void testParse_PeopleProtocol() throws Exception
    {
        ...
        when(AsnSchemaTypeDefinitionParser
                .parse("Person", "SEQUENCE { firstName [1] OCTET STRING, lastName [2] OCTET STRING, title [3] ENUMERATED { mr, mrs, ms, dr, rev } OPTIONAL, gender OPTIONAL }"))
                .thenReturn(mockedAsnSchemaTypeDefinitions.get(1));

        when(AsnSchemaTypeDefinitionParser
                .parse("Gender", "ENUMERATED { male(0), female(1) }"))
                .thenReturn(mockedAsnSchemaTypeDefinitions.get(2));

        final AsnSchemaModule actualModule = AsnSchemaModuleParser.parse(MockAsnSchemaModule.TEST_MODULE_PEOPLE_PROTOCOL);
        assertNotNull(actualModule);
    }
}
