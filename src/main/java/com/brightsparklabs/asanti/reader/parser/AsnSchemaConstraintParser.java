...
public class AsnSchemaConstraintParser
{
    ...
    private static final Pattern PATTERN_SIZE_CONSTRAINT = Pattern.compile(
            "\\(?\\s*SIZE\\s*\\(\\s*(\\d+)\\s*\\.\\.\\s*(\\d+)[^\\)]*\\)\\s*\\)?");

    /** pattern to match an exact SIZE constraint */
    private static final Pattern PATTERN_EXACT_SIZE_CONSTRAINT = Pattern.compile(
            "\\(?\\s*SIZE\\s*\\(\\s*(\\d+)\\s*\\)\\s*\\)?");

    /** pattern to match a bounded numeric value constraint */
    private static final Pattern PATTERN_NUMERIC_VALUE_CONSTRAINT = Pattern.compile(
            "\\(?\\s*(-?\\d+)\\s*\\.\\.\\s*(-?\\d+)[^\\)]*\\)?");

    /** pattern to match an exact numeric value constraint */
    private static final Pattern PATTERN_EXACT_NUMERIC_VALUE_CONSTRAINT = Pattern.compile(
            "\\(?\\s*(-?\\d+)\\s*\\)?");
     ...
     *
     * @param matcher
     *         matcher which matched on {@link #PATTERN_EXACT_SIZE_CONSTRAINT}
     *
     ...
     *
     * @param matcher
     *         matcher which matched on {@link #PATTERN_NUMERIC_VALUE_CONSTRAINT}
     *
     ...
     *
     * @param matcher
     *         matcher which matched on {@link #PATTERN_EXACT_NUMERIC_VALUE_CONSTRAINT}
     *
