...
/**
 * Modern streaming-based BER (Basic Encoding Rules) data parser.
 *
 ...
 *       primitives
 *   <li><b>UNIVERSAL Tag Injection:</b> Adds UNIVERSAL tags for untagged elements in sequences/sets
 * </ul>
 *
 ...
 */
public class AsnBerDataStreamReader {
    // -------------------------------------------------------------------------
    // CONSTANTS
    // -------------------------------------------------------------------------
    ...
    public static Stream<RawAsnData> read(final InputStream inputStream) {
        ...
        return Stream.<RawAsnData>generate(
                        () -> {
                            try {
                                ...
                                // Pre-sized map for typical PDU (~50 tags)
                                ...
                                // level
                                // The root is treated as if already inside a tag to prevent adding
                                // UNIVERSAL prefix
     ...
     *     children)
     * @param insideTag true if we're already inside a tag context (prevents adding duplicate
     *     UNIVERSAL tags)
    ...
    private static void extractElements(
        ...
        switch (derObject) {
            case ASN1TaggedObject taggedObject -> {
                ...
                // INTEGER)
                // In such cases, the BER encoding includes both the context tag and the UNIVERSAL
                // tag
                if (taggedObject.isExplicit() && baseObject != null) {
                    ...
                    if (!(baseObject instanceof ASN1TaggedObject)) {
                        ...
                        if (tagNo > 0 && tagNo < 0x1F) { // Valid tag, not high-tag-number form
     ...
     * @param insideTag true if already inside a tag context (skips adding UNIVERSAL tag)
     ...
     */
    private static int getUniversalTagNumber(final ASN1Primitive primitive) {
        return switch (primitive) {
