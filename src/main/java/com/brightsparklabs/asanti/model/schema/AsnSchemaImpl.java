...
public class AsnSchemaImpl implements AsnSchema {
    ...
    public ImmutableSet<OperationResult<DecodedTag, String>> getDecodedTags(
        ...
        for (String rawTag : rawTags) {
            final OperationResult<DecodedTag, String> decodeResult =
                    getDecodedTag(rawTag, topLevelTypeName, session);
     ...
     */
    private OperationResult<DecodedTag, String> getDecodedTag(
            String rawTag, String topLevelTypeName, DecodingSession session) {
        ...
        final DecodedTagsAndType decodedTagsAndType =
                decodeTags(tags.iterator(), typeDefinition.getType(), session);
     ...
     *
     * @param rawTags raw tags to decode. This should be an iterable in the order of the tags. E.g.
     *     The raw tag {code "/1[0]/0[1]/1[2]"} should be provided as an iterator of {code ["1[0]",
     *     "0[1]", "1[2]"]}
     * @param containingType the {@link AsnSchemaType} that is the parent of the chain to be decoded
     * @return a list of all decoded tags. If a raw tag could not be decoded then processing stops.
     *     E.g. for the raw tags {code "1", "0" "1", "99", "98"}, if the {@code "99"} raw tag cannot
     *     be decoded, then a list containing the decoded tags for only the first three raw tags is
     *     returned (e.g. {@code ["Header", "Published", "Date"]})
     */
    private DecodedTagsAndType decodeTags(
