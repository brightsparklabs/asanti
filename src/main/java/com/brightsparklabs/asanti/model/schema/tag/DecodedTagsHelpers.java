...
public class DecodedTagsHelpers
{
     ...
     *
     * @return just the name of the immediate children, this is trimmed of any index (eg [1]),  ie
     * if parentTag is "/Parent/blah" then /Parent/blah/someTag[0] turns in to someTag.  If
    ...
    public static ImmutableSet<String> getImmediateChildren(AsnData asnData, String parentTag)
    {
        ...
        return ImmutableSet.copyOf(Collections2.filter(transform(filtered,
                new TransformToJustChildName(parentTag)), filterOutDigits));
    }
     ...
     *
     * Example, someTag[1] will return someTag.
     *
     * @param tag
     *         the tags to strip
     *
    ...
    private static class OnlyStartsWith implements Predicate<String>
    {
         ...
         *
         * @param parentTag
         *         the parent tag we are finding children for
         */
     ...
     *
     * Will return an empty string if the input does not start with the expected parent tag, or does
     * not have any child
     */
